...
namespace Team15_FinalProject.Controllers
{
    public class TransactionsController : Controller
    {
        ...
        public ActionResult Deposit([Bind(Include = "ProductID,ProductName,ProductType,ProductNumber,ProductString,ProductBalance")] Account product, int DesProductID, decimal DesAmount, string DesComment, DateTime desdate)
        {
            if (ModelState.IsValid)
            {
                ...
                if (product.ProductType==ProductType.IRA)
                {
                    ...
                    {
                        return RedirectToAction("Error", "IRAs");
                    }
                    else
                    {
                ...
                Transaction trans = new Transaction { Description = "Deposit of $" + DesAmount + " into " + product.ProductName + ".", TransactionType = TransactionType.deposit, Amount = DesAmount, Date = desdate, TransactionNumber = NextTransNum, Comment = DesComment };
            ...
            if (ModelState.IsValid)
            {
                ...
                if (product.ProductType == ProductType.IRA)
                {
                    ...
                    if (age > 65)
                    {
                        ...
                        db.SaveChanges();
                        product.ProductName = ira.IRAName;
                        Int32 NextTransNumber = db.Transactions.ToList().Count() + 1;
                        if (DesComment == null || DesComment == "")
                        {
                            DesComment = "No comment entered.";
                        }
                        Transaction transact = new Transaction { Description = "Withdrawal of $" + DesAmount + " from " + product.ProductName + ".", TransactionType = TransactionType.withdrawal, Amount = DesAmount, Date = desdate, TransactionNumber = NextTransNumber, Comment = DesComment };
                ...
                Transaction trans = new Transaction { Description = "Withdrawal of $" + DesAmount + " from " + product.ProductName + ".", TransactionType = TransactionType.withdrawal, Amount = DesAmount, Date = desdate, TransactionNumber = NextTransNum, Comment = DesComment };
                db.Transactions.Add(trans);
                db.SaveChanges();
                return RedirectToAction("Index", "Home");
            }
        ...
        public ActionResult IRAWithdrawInclude()
        {
            ...
            TVM.Amount -= 30;
            Transaction trans = new Transaction { Description = "Withdrawal of $" + TVM.Amount + " from " + destination.ProductName + ".", TransactionType = TransactionType.withdrawal, Amount = TVM.Amount, Date = TVM.Date, TransactionNumber = NextTransNum, Comment = TVM.Comment };
            Int32 NextTransNum2 = NextTransNum + 1;
            Transaction fee = new Transaction { Description = "Fee of $30 incurred during withdrawal from " + destination.ProductName + ".", TransactionType = TransactionType.fee, Amount = 30, Date = TVM.Date, TransactionNumber = NextTransNum2, Comment = TVM.Comment };
            db.Transactions.Add(trans);
        ...
        public ActionResult Confirmed()
        {
            ...
            Transaction trans = new Transaction { Description = "Transfer of $" + TVM.Amount + " from " + origin.ProductName + " into " + destination.ProductName + ".", TransactionType = TransactionType.transfer, Amount = TVM.Amount, Date = TVM.Date, TransactionNumber = NextTransNum, Comment = TVM.Comment };
            if (TVM.Overdraft == true)
            {
                NextTransNum += 1;
                Transaction OverdraftFee = new Transaction { Description = "Overdraft Fee.", TransactionType = TransactionType.fee, Amount = 30, Date = TVM.Date, TransactionNumber = NextTransNum, Comment = "" };
        ...
        public ActionResult IRATransferInclude()
        {
            ...
            Transaction trans = new Transaction { Description = "Transfer of $" + TVM.Amount + " from " + origin.ProductName + " into " + destination.ProductName + ".", TransactionType = TransactionType.transfer, Amount = TVM.Amount, Date = TVM.Date, TransactionNumber = NextTransNum, Comment = TVM.Comment };
            NextTransNum += 1;
            Transaction fee = new Transaction { Description = "Fee of $30 incurred during transfer from " + origin.ProductName + " to " + destination.ProductName + ".", TransactionType = TransactionType.fee, Amount = 30, Date = TVM.Date, TransactionNumber = NextTransNum, Comment = TVM.Comment };
            db.Transactions.Add(trans);
            db.Transactions.Add(fee);
            if (TVM.Overdraft == true)
            {
        ...
        public SelectList GetAllProducts()
        {
            var query = from x in db.Accounts
                        select x;
            List<Account> allProducts = query.ToList();
